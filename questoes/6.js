...
produtos.forEach(p => {
  if (p.tipo == TIPO_PRODUTO.BEBIDA) {
    ...
    divRadio.classList.add("radio-inline");
    divRadio.append(label);
    document.querySelector("#bebidas").append(divRadio);
    checked = false;
  } else {
    ...
    divCheck.classList.add("checkbox-inline");
    divCheck.append(label);
    document.querySelector("#doces-salgados").append(divCheck);
  }
});
